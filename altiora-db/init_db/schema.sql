...
CREATE TYPE property_status   AS ENUM ('available', 'sold_out', 'paused'); -- paused: propiedades que la agencia aun no quiere mostrar
...
CREATE TABLE properties (
    ...
    floor_number    SMALLINT, -- para apartamentos
    total_floors    SMALLINT        DEFAULT 0, -- aqui si se cuentan casas, condominios y locales comerciales
    year_built      SMALLINT,
    -- Ubicación
    province        VARCHAR(20),
    city            VARCHAR(40), -- cantón
    district        VARCHAR(40), -- ciudad
    address         VARCHAR(100),
    latitude        NUMERIC(10, 7), -- mostrar en mapas
    ...
    -- Control
    views_count     INT             NOT NULL DEFAULT 0, -- populares
    published_at    TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    updated_at      TIMESTAMPTZ     NOT NULL DEFAULT NOW()
);

-- minimo de 5, maximo de 15 imagenes por propiedad
...
--  Solicitud de compra de un buyer sobre una propiedad
--  Una propiedad solo puede tener 1 request activo a la vez
-- ============================================================
...
CREATE TABLE purchase_requests (
    ...
    message             TEXT,           -- mensaje opcional del comprador
    rejection_reason    TEXT,           -- razón del rechazo por la agencia
    -- Snapshot del precio al momento de la solicitud
    ...
    responded_at        TIMESTAMPTZ,                        -- cuando agencia acepta/rechaza
    created_at          TIMESTAMPTZ     NOT NULL DEFAULT NOW(),
    updated_at          TIMESTAMPTZ     NOT NULL DEFAULT NOW(),

    -- Solo 1 request pending o accepted por propiedad a la vez
...
--  Reseñas de compradores sobre agencias
--  Un comprador solo puede tener 1 review por agencia (editable)
-- ============================================================
...
CREATE TABLE agency_reviews (
    ...
    -- Un comprador solo da una review por agencia (puede dar reviews a otras agencias)
...
CREATE TABLE email_logs (
    ...
    template_name       VARCHAR(100),   -- ej: 'purchase_confirmed', 'review_rejected', 'account_banned'
    related_entity      VARCHAR(50),    -- ej: 'purchase_request', 'review_request', 'sell_request'
...
-- Búsquedas frecuentes en propiedades
...
-- Aplicar trigger a todas las tablas con updated_at cada que una se actualice (solo se aplica a la actualizada)
DO $$
DECLARE
    t TEXT;
BEGIN
...
-- ============================================================
--  VISTA: Agencies rating summary (útil para la API)
-- ============================================================
...
-- ============================================================
--  VISTA: Properties details (útil para la API)
-- ============================================================
